...
async function upsertVideo(supabase, videoId, fileName, driveFileId, segments, title = null, videoDriveId = null) {
  const totalDuration = segments[segments.length - 1]?.end_ms ?? 0;
  ...
  const { error } = await supabase.from('videos').upsert({
    id: videoId,
    file_name: fileName,
    drive_file_id: driveFileId,      // transcript JSON's Drive file ID
    video_drive_id: videoDriveId,    // source video's Drive file ID → used for direct link
...
async function isAlreadyIngested(supabase, videoId, driveFileId) {
  // Check by drive_file_id first (stable even if filename changes)
  if (driveFileId) {
  ...
  // Fallback: check by video ID (filename-based)
  const { data } = await supabase
...
async function main() {
  ...
  for (const file of files) {
    ...
    // Skip if already ingested (check by stable drive_file_id, not filename)
    if (await isAlreadyIngested(supabase, videoId, file.id)) {
    ...
    try {
      ...
      // 2. Get title and video_drive_id from JSON (no renaming, no GPT title gen)
      ...
      await upsertVideo(supabase, videoId, file.name, file.id, segments, title, videoDriveId);
